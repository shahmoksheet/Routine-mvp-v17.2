...
export default function CalendarView() {
  ...
  const [calendarSource, setCalendarSource] = useState<'routine' | 'google' | 'outlook'>('routine');
    ...
    <div className="flex flex-col h-full bg-[#f8fafc] transition-colors">
      ...
      <div className="px-5 py-4 space-y-3">
        <div className="flex items-center justify-center bg-slate-200/50 rounded-2xl p-1.5 shadow-inner">
          <button 
            onClick={() => setCalendarSource('routine')}
            className={`flex-1 py-1.5 text-xs font-bold rounded-xl transition-all ${calendarSource === 'routine' ? 'bg-white shadow-sm text-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Routine
          </button>
          <button 
            onClick={() => setCalendarSource('google')}
            className={`flex-1 py-1.5 text-xs font-bold rounded-xl transition-all ${calendarSource === 'google' ? 'bg-white shadow-sm text-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Google
          </button>
          <button 
            onClick={() => setCalendarSource('outlook')}
            className={`flex-1 py-1.5 text-xs font-bold rounded-xl transition-all ${calendarSource === 'outlook' ? 'bg-white shadow-sm text-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Outlook
          </button>
        </div>
      ...
      <main className="flex-1 overflow-y-auto pb-24 no-scrollbar">
        ...
        <section className="mt-8 px-5">
          ...
          <div className="p-8 pb-24 text-center mt-12 bg-white rounded-3xl mx-5 border border-slate-100 shadow-soft">
             <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-6">
               <CalendarIcon className="w-10 h-10 text-blue-500" />
             </div>
             <h3 className="text-xl font-black text-slate-800 tracking-tight mb-2">Google Calendar</h3>
             <p className="text-sm font-medium text-slate-500 mb-8 max-w-sm mx-auto">
               You can sync your Routine tasks to Google Calendar by subscribing to our calendar feed, or open Google Calendar directly on your device.
             </p>
             <div className="space-y-4 max-w-sm mx-auto">
               <button 
                 onClick={() => window.open('https://calendar.google.com/calendar/r', '_blank')}
                 className="w-full flex items-center justify-center gap-2 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-bold transition-all shadow-lg shadow-blue-500/30"
               >
                 Open Google Calendar
               </button>
               <button className="w-full py-3 text-sm font-bold text-slate-500 hover:text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">
                 Get ICS Subscription Link
               </button>
             </div>
          </div>
        ) : (
          <div className="p-8 pb-24 text-center mt-12 bg-white rounded-3xl mx-5 border border-slate-100 shadow-soft">
             ...
             <h3 className="text-xl font-black text-slate-800 tracking-tight mb-2">Outlook Calendar</h3>
             <p className="text-sm font-medium text-slate-500 mb-8 max-w-sm mx-auto">
               You can sync your Routine tasks to Outlook Calendar by subscribing to our calendar feed, or open Outlook Calendar directly on your device.
             </p>
             <div className="space-y-4 max-w-sm mx-auto">
               <button 
                 onClick={() => window.open('https://outlook.live.com/calendar/0/view/month', '_blank')}
                 className="w-full flex items-center justify-center gap-2 py-3 bg-sky-500 hover:bg-sky-600 text-white rounded-xl font-bold transition-all shadow-lg shadow-sky-500/30"
               >
