...
export default function SettingsView() {
  ...
  const handleAction = (action: string) => {
    if (action === 'logout') {
      ...
      setActiveModal('Notifications Settings');
    ...
    } else if (action === 'RecurringTemplates') {
      setCurrentView('recurring-templates');
    } else if (action === 'KanbanCustomization') {
      setCurrentView('kanban-customization');
    } else if (action === 'Sync') {
      toast.promise(new Promise(resolve => setTimeout(resolve, 1500)), {
        loading: 'Syncing with Routine...',
      ...
      <div className="flex-1 overflow-y-auto pb-24 no-scrollbar px-4 pt-4">
        ...
        {(canUpdateWorkspace || canManageMembers || canManageRoles) && (
          <SettingsGroup title="Workspace & Team">
            ...
            {canUpdateWorkspace && <SettingsItem icon={LayoutTemplate} label="Recurring Task Templates" onClick={() => handleAction('RecurringTemplates')} />}
          ...
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm shadow-2xl border border-slate-100 transition-colors relative overflow-hidden">
            ...
            {activeModal === 'Delete Workspace' ? (
              ...
              <div className="relative z-10 text-center">
                ...
                <h3 className="text-xl font-black text-slate-800 mb-2">Deactivate Account?</h3>
                <p className="text-slate-500 text-sm font-medium mb-8">
                  This will temporarily deactivate your account and pause all related background tasks like recurring schedules. You will not receive any notifications and gibberish data will not be populated. You can reactivate by signing in later.
                </p>
                <div className="flex flex-col gap-3">
                  <button 
                    disabled={isDeleting}
                    onClick={handleDeactivateAccount}
                    className="w-full py-4 bg-amber-500 text-white font-black rounded-2xl hover:bg-amber-600 transition-all shadow-glow active:scale-95 disabled:opacity-50"
                  >
            ...
            ) : activeModal === 'Notifications Settings' ? (
              <div className="relative z-10 text-left">
                <div className="flex items-center gap-3 mb-6 relative z-10">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-500">Manage your alerts</p>
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-8 relative z-10">
                  <div className="flex items-center justify-between group">
                    ...
                    <div 
                      ...
                      className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${pushNotifs ? 'bg-primary-600' : 'bg-slate-200'}`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${pushNotifs ? 'left-7' : 'left-1'}`}></div>
                    </div>
                  </div>
                  
                  <div className="flex items-center justify-between group">
                    <div>
                      <h4 className="text-sm font-bold text-slate-800">Email Updates</h4>
                      <p className="text-xs text-slate-500">Daily summaries and reports</p>
                    </div>
                    <div 
                      onClick={() => setEmailNotifs(!emailNotifs)}
                      className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${emailNotifs ? 'bg-primary-600' : 'bg-slate-200'}`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${emailNotifs ? 'left-7' : 'left-1'}`}></div>
                    </div>
                  </div>

                  <div className="flex items-center justify-between group">
                    <div>
                      <h4 className="text-sm font-bold text-slate-800">Task Mentions</h4>
                      <p className="text-xs text-slate-500">When someone tags you</p>
                    </div>
                    <div 
                      onClick={() => setMentionNotifs(!mentionNotifs)}
                      className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${mentionNotifs ? 'bg-primary-600' : 'bg-slate-200'}`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${mentionNotifs ? 'left-7' : 'left-1'}`}></div>
                    </div>
                  </div>
                  
                  <div className="flex items-center justify-between group">
                    <div>
                      <h4 className="text-sm font-bold text-slate-800">Offline Sync Alerts</h4>
                      <p className="text-xs text-slate-500">Notify when back online</p>
                    </div>
                    <div 
                      onClick={() => setSyncAlerts(!syncAlerts)}
                      className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${syncAlerts ? 'bg-primary-600' : 'bg-slate-200'}`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${syncAlerts ? 'left-7' : 'left-1'}`}></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-3 relative z-10">
                  <button 
                    ...
                    className="flex-1 py-4 bg-primary-600 text-white font-black rounded-2xl hover:bg-primary-700 transition-all shadow-glow active:scale-95"
                  >
                    Save
                  </button>
                  <button 
