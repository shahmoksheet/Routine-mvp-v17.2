...
import { Layers, Mail, Lock, Phone, ArrowRight, CheckCircle2, UserPlus, Bot } from 'lucide-react';
...
export function AuthView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        if (!isLogin && !isInviteFlow) {
          localStorage.setItem('firstTimeOnboarding', 'true');
        }
        login(data.user, data.token);
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-slate-950 flex flex-col justify-center p-6 relative overflow-hidden transition-colors duration-500">
      <div className="absolute top-[-20%] left-[-10%] w-[120%] h-96 bg-primary-500 rounded-b-[100%] blur-3xl opacity-10"></div>
      
      <div className="w-full max-w-md mx-auto z-10 transition-colors">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 border border-slate-100 dark:border-slate-800 relative overflow-hidden transition-colors">
          <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-primary-50 dark:from-primary-900/20 to-transparent rounded-bl-full opacity-50 pointer-events-none"></div>
          
          <h2 className="text-3xl font-black text-slate-800 dark:text-white mb-2 text-center tracking-tight relative z-10 transition-colors">
                ...
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => {
                      localStorage.setItem('firstTimeOnboarding', 'true');
                      login({ id: 'demo123', name: 'New Owner', email: 'owner@newbiz.com', role: 'owner' }, 'fake-token');
                    }} className="text-xs font-bold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-2 rounded-xl transition-colors col-span-2 border border-indigo-100 uppercase tracking-widest flex items-center justify-center gap-2">
                      <span className="w-4 h-4 bg-indigo-600 rounded-full flex items-center justify-center"><Bot className="w-3 h-3 text-white" /></span>
                      Demo: AI Workspace Setup
                    </button>
                    <button type="button" onClick={() => setFormData({...formData, email: 'moksheet77@gmail.com', password: 'password123'})} className="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-xl transition-colors">Owner (Moksheet)</button>
                    <button type="button" onClick={() => setFormData({...formData, email: 'rohit@demo.com', password: 'password123'})} className="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-xl transition-colors">Manager (Rohit)</button>
                    <button type="button" onClick={() => setFormData({...formData, email: 'jasprit@demo.com', password: 'password123'})} className="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-xl transition-colors col-span-2">Manager (Bumrah)</button>
                    <button type="button" onClick={() => setFormData({...formData, email: 'jadeja@demo.com', password: 'password123'})} className="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-xl transition-colors">Staff (Jadeja)</button>
                    <button type="button" onClick={() => setFormData({...formData, email: 'rahul@demo.com', password: 'password123'})} className="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-xl transition-colors">Staff (Rahul)</button>
                    <button type="button" onClick={() => setFormData({...formData, email: 'pant@demo.com', password: 'password123'})} className="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-xl transition-colors">Staff (Pant)</button>
                    <button type="button" onClick={() => setFormData({...formData, email: 'surya@demo.com', password: 'password123'})} className="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-xl transition-colors">Staff (Surya)</button>
                  </div>
                </div>
              )}
            </div>
