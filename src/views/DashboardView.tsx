...
export default function DashboardView() {
  ...
  const relevantTasks = useMemo(() => {
    ...
    if (user?.role === 'Manager') {
      ...
      if (dateRange !== 'all' && t.dueDate) {
        const taskDate = new Date(t.dueDate);
        const now = new Date();
        if (dateRange === '7d' && taskDate < subDays(now, 7)) return false;
        if (dateRange === '30d' && taskDate < subDays(now, 30)) return false;
        if (dateRange === 'custom' && customStart && customEnd) {
           if (taskDate < new Date(customStart) || taskDate > new Date(customEnd)) return false;
        }
      }
