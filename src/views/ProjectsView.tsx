...
export default function ProjectsView() {
  ...
  const getProjectTasks = (projectTitle: string) => {
    return tasks.filter(t => !t.isDeactivated && t.project === projectTitle);
  };
  ...
  const confirmDelete = async () => {
    if (projectToDelete) {
      const projectTasks = tasks.filter(t => !t.isDeactivated && t.project === projectToDelete.title && t.status !== 'Completed');
...
function ProjectCard({ project, tasks, departments, onEdit, onDelete, onClick }: { project: Project, tasks: any[], departments: any[], onEdit: (e: React.MouseEvent) => void, onDelete: (e: React.MouseEvent) => void, onClick: () => void }) {
  ...
  const projectTasks = tasks.filter(t => !t.isDeactivated && t.project === project.title);
