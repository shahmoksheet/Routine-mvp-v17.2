...
export default function DraftsView() {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('AI successfully structured the draft!');
        setRawText('');
        fetchDrafts();
      } else {
        const errorData = await res.json();
        toast.error(errorData.error || 'Failed to parse text');
      }
    } catch (error) {
      toast.error('AI Extraction failed. Check API configuration.');
    } finally {
      setIsParsing(false);
    }
  };
    ...
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <main className="flex-1 overflow-y-auto p-5 pb-24">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-500 font-medium">Review and confirm tasks created via WhatsApp or use AI to parse a thought.</p>
        </div>

        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm mb-8">
          <label className="block text-sm font-bold text-slate-700 mb-2">Smart Capture (AI Powered)</label>
          <textarea
            value={rawText}
            onChange={(e) => setRawText(e.target.value)}
            placeholder="e.g. Type something informal like 'Remind John to fix the marketing page tomorrow. It is super urgent.'"
            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent min-h-[100px] resize-none"
            disabled={isParsing}
          />
          <div className="mt-3 flex justify-end">
            <button
              ...
              className="px-6 py-2 bg-primary-600 text-white text-sm font-bold rounded-xl hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
              {isParsing ? 'Extracting...' : 'Parse into Draft'}
            </button>
          </div>
        </div>
              ...
              <div key={draft.id} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all">
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-100">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold text-sm text-slate-600 bg-slate-100 dark:bg-slate-700/50 dark:text-slate-300 dark:hover:bg-slate-700 hover:bg-slate-200 transition-colors"
                  >
                    <X className="w-4 h-4" />
                    Discard
                  </button>
      ...
      {selectedDraft && (
        <CreateTaskModal
          ...
          initialData={{
            ...
            dueDate: selectedDraft.due_date || undefined
          }}
        />
      )}
    </div>
