...
import { BarChart2, PieChart as PieChartIcon, TrendingUp, Users, Filter, Calendar } from 'lucide-react';
...
export default function ReportsView() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (selectedMemberId !== 'all') {
      result = result.filter(t => t.assignees && t.assignees.includes(selectedMemberId));
    }
    if (dateRange !== 'all') {
      const now = new Date();
      result = result.filter(t => {
        ...
        if (dateRange === '7d') return taskDate >= subDays(now, 7);
        if (dateRange === '30d') return taskDate >= subDays(now, 30);
        if (dateRange === '90d') return taskDate >= subDays(now, 90);
        if (dateRange === '365d') return taskDate >= subDays(now, 365);
        if (dateRange === 'custom' && customStart && customEnd) {
          return taskDate >= new Date(customStart) && taskDate <= new Date(customEnd);
        }
        return true;
      });
    }
    return result;
  }, [tasks, selectedMemberId, dateRange, customStart, customEnd]);
