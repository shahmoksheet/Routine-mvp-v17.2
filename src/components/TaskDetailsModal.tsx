...
export default function TaskDetailsModal({ taskId, onClose, onOpenTask }: { taskId: string, onClose: () => void, onOpenTask?: (taskId: string) => void }) {
  ...
  const getGoogleCalendarUrl = () => {
    ...
    const startTimeStr = task.startTime || '00:00';
    const endStr = task.dueDate || startStr;
    const endTimeStr = task.dueTime || '23:59';
    
    const formattedStart = formatDate(startStr, startTimeStr);
    const formattedEnd = formatDate(endStr, endTimeStr);

    return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${title}&details=${details}&dates=${formattedStart}/${formattedEnd}`;
  };
  ...
  const getOutlookCalendarUrl = () => {
    ...
    return `https://outlook.live.com/calendar/0/deeplink/compose?path=/calendar/action/compose&rru=addevent&subject=${title}&body=${details}&startdt=${startStr}T${startTimeStr}&enddt=${endStr}T${endTimeStr}`;
  };
    ...
    >
      ...
      <main className="flex flex-col gap-6 p-5 pb-32">
        ...
        <section className="flex flex-wrap gap-2">
          {(!task.recurring || task.recurring === 'None') && task.dueDate ? (
            <>
              <button 
                onClick={() => {
                  window.open(getGoogleCalendarUrl(), '_blank');
                }}
                className="flex items-center gap-2 bg-blue-50 text-blue-600 px-3 py-2 rounded-xl text-xs font-bold hover:bg-blue-100 transition-colors"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 bg-sky-50 text-sky-600 px-3 py-2 rounded-xl text-xs font-bold hover:bg-sky-100 transition-colors"
              >
                <Calendar className="w-4 h-4" /> Add to Outlook
              </button>
            </>
          ) : null}
        </section>
