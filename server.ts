...
function getAiClient() {
  if (!aiClient) {
    ...
    if (!apiKey) {
      console.warn('GEMINI_API_KEY is missing, trying default instantiation');
    }
...
async function startServer() {
  ...
  // --- TEMPLATES API ---
  app.get('/api/templates', async (req: AuthRequest, res) => {
    const { workspaceId } = req.user!;
    const templates = await db.prepare('SELECT * FROM templates WHERE workspace_id = ?').all(workspaceId);
    res.json(templates);
  });

  app.post('/api/templates', async (req: AuthRequest, res) => {
    const { workspaceId, role } = req.user!;
    if (!['Owner', 'Admin'].includes(role)) return res.status(403).json({ error: 'Forbidden' });
    
    const { template_name, description, project_id, recurrence_rule } = req.body;
    const templateId = generateId('TPL');
    
    db.prepare(`
      INSERT INTO templates (id, workspace_id, template_name, description, project_id, recurrence_rule)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(templateId, workspaceId, template_name, description || null, project_id || null, recurrence_rule || null);
    
    res.json({ success: true, id: templateId });
  });

  app.patch('/api/templates/:id', async (req: AuthRequest, res) => {
    const { workspaceId, role } = req.user!;
    if (!['Owner', 'Admin'].includes(role)) return res.status(403).json({ error: 'Forbidden' });
    ...
    const updates: string[] = [];
    const params: any[] = [];
    if (template_name !== undefined) { updates.push('template_name = ?'); params.push(template_name); }
    if (description !== undefined) { updates.push('description = ?'); params.push(description); }
    ...
    if (updates.length > 0) {
      ...
      await db.prepare(`UPDATE templates SET ${updates.join(', ')} WHERE id = ? AND workspace_id = ?`).run(...params);
    }
    res.json({ success: true });
  });

  app.delete('/api/templates/:id', async (req: AuthRequest, res) => {
    const { workspaceId, role } = req.user!;
    const { id } = req.params;
    if (!['Owner', 'Admin'].includes(role)) return res.status(403).json({ error: 'Forbidden' });
    
    await db.prepare('DELETE FROM templates WHERE id = ? AND workspace_id = ?').run(id, workspaceId);
    res.json({ success: true });
  });
  ...
  app.post('/api/drafts/parse', authMiddleware, async (req: AuthRequest, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Parse the following raw text or voice snippet into structured task data.\n\nRaw Input: "${text}"\n\nDo your best to infer the priority (High, Medium, Low) based on urgency words. If a due date or time is explicitly stated or can be reasonably inferred, extract it as YYYY-MM-DD. Set due_time as HH:mm if any.`,
        config: {
          responseMimeType: 'application/json',
          responseSchema: {
            type: "OBJECT",
            properties: {
              title: { type: "STRING", description: "A concise title for the task (max 60 chars)" },
              description: { type: "STRING", description: "Detailed description based on input" },
              priority: { type: "STRING", description: "High, Medium, or Low" },
              due_date: { type: "STRING", description: "Extracted due date (YYYY-MM-DD) or null" },
              due_time: { type: "STRING", description: "Extracted due time (HH:mm) or null" },
            },
  ...
  app.get('/api/drafts', authMiddleware, async (req: AuthRequest, res) => {
    const { id: userId, workspaceId, role } = req.user!;
    try {
      let drafts;
      if (role === 'Owner') {
        drafts = db.prepare('SELECT * FROM draft_tasks WHERE workspace_id = ? AND status = ? ORDER BY created_at DESC').all(workspaceId, 'pending_confirmation');
      } else {
        drafts = db.prepare('SELECT * FROM draft_tasks WHERE created_by = ? AND workspace_id = ? AND status = ? ORDER BY created_at DESC').all(userId, workspaceId, 'pending_confirmation');
      }
      res.json(drafts);
    } catch (error) {
  ...
  app.post('/api/tasks', async (req: AuthRequest, res) => {
    const { id: userId, role, workspaceId, permissions } = req.user!;
    const { id: reqId, title, description, project, priority, status, startDate, startTime, dueDate, dueTime, type, assignees, assignedToType, proofType, photoUrl, attachments, recurring, timeSpent, comments, subtasks, dependencies, relatedTasks, requiresApproval, geofence, targetWorkspaces } = req.body;
    
    let targetWsIds = [workspaceId];
    if (targetWorkspaces && Array.isArray(targetWorkspaces) && targetWorkspaces.length > 0) {
      if (permissions && permissions.includes('switch_workspace')) {
        targetWsIds = targetWorkspaces;
      }
    }
    ...
    const stmt = db.prepare(`
      INSERT INTO tasks (
        id, workspace_id, title, description, project_id, priority, status, start_date, start_time, due_date, due_time, type, created_by, assigned_to_type, assigned_to_id, 
    ...
    for (const targetWsId of targetWsIds) {
      const id = targetWsIds.length === 1 && reqId ? reqId : generateId('TSK');
      await stmt.run(
        id, targetWsId, title, description || null, project, priority, status, startDate || null, startTime || null, dueDate || null, dueTime || null, type || 'task', userId, finalAssignedToType, assignedToId,
      ...
      const newTask = { ...req.body, id, workspace_id: targetWsId, workspaceId: targetWsId, createdBy: userId, assignees: assignedToId ? [assignedToId] : [], assignedToType: finalAssignedToType };
      createdTasks.push(newTask);
      io.emit('task_created', newTask);
      await logActivity(targetWsId, userId, 'create_task', { taskId: id, title });
      ...
      if (assignedToId && assignedToId !== userId) {
        if (finalAssignedToType === 'user') {
          ...
          db.prepare('INSERT INTO messages (id, workspace_id, sender_id, receiver_id, message) VALUES (?, ?, ?, ?, ?)')
            .run(msgId, targetWsId, userId, assignedToId, msgText);
          
          const savedMsg = { id: msgId, workspace_id: targetWsId, sender_id: userId, receiver_id: assignedToId, message: msgText, timestamp: new Date().toISOString() };
          io.to(`user_${assignedToId}`).to(`user_${userId}`).emit('chat_message', savedMsg);

          const notifId = generateId('NOT');
          db.prepare('INSERT INTO notifications (id, user_id, title, message, type) VALUES (?, ?, ?, ?, ?)')
            .run(notifId, assignedToId, 'New Task Assigned', `${sender?.name || 'Someone'} assigned you a new task: ${title} in workspace ${targetWsId}`, 'info');
          sendNotification(assignedToId, 'New Task Assigned', `${sender?.name || 'Someone'} assigned you a new task: ${title} in workspace ${targetWsId}`, 'info');
    ...
    res.json({ success: true, task: createdTasks[0] });
  });
  ...
  app.get('/api/time-logs/active', async (req: AuthRequest, res) => {
    ...
    res.json({ active: true, ...activeTimer });
  });
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Serve public directory explicitly during development before vite middleware
  ...
  // Global error handler to ensure JSON responses on API failures
